...
form.addEventListener("submit", async event => {
  ...
  if (!GOOGLE_SHEETS_WEB_APP_URL) {
    msg.textContent =
      "Google Sheets n'est pas encore configuré dans config.js.";
    return;
  }
  ...
  try {
    ...
    if (result.success === false) {
      throw new Error(result.message || "Google Sheets a refusé la demande");
    }

    document.getElementById("ref").textContent = data.reference;
    form.classList.add("hidden");
    document.getElementById("success").classList.remove("hidden");
  } catch (error) {
    console.error(error);
    msg.textContent =
      "Échec de l'envoi vers Google Sheets. Vérifiez l'URL /exec dans config.js.";
  } finally {
